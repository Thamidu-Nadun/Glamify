...
const appointments = [
  { id: 1, client: 'Emma Watson', service: 'Hair Coloring', time: '10:00 AM', date: '2025-05-05', stylist: 'Jennifer' },
  { id: 2, client: 'Sophia Miller', service: 'Haircut & Styling', time: '11:30 AM', date: '2025-05-05', stylist: 'Robert' },
  { id: 3, client: 'James Smith', service: 'Beard Trim', time: '1:00 PM', date: '2025-05-05', stylist: 'Michael' },
  { id: 4, client: 'Olivia Brown', service: 'Manicure', time: '2:30 PM', date: '2025-05-05', stylist: 'Lisa' },
  { id: 5, client: 'Alice Cooper', service: 'Facial', time: '3:45 PM', date: '2025-05-05', stylist: 'Diana' },
];
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div 
        ...
        } duration-300 h-screen bg-gradient-to-b from-purple-800 to-pink-500 text-white fixed left-0 top-0 transition-all z-20`}
      >
        <div className="flex items-center justify-between p-4 border-b border-purple-700">
          ...
          <button onClick={toggleSidebar} className="p-2 rounded-lg hover:bg-purple-700">
            <Menu size={24} />
          </button>
        </div>
                ...
                <div>
                  <button
                    ...
                    className={`flex items-center ${
                      ...
                      activeMenu === item.id ? "bg-purple-700" : "hover:bg-purple-700/50"
                    }`}
                  >
                    <div className={`flex items-center ${!isSidebarOpen ? "justify-center" : ""}`}>
                      <span className={`${!isSidebarOpen ? "mr-0" : "mr-3"}`}>{item.icon}</span>
                      {isSidebarOpen && <span>{item.label}</span>}
                    </div>
                    {isSidebarOpen && item.submenus && (
                      <span>
                        {submenuOpen[item.id] ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                      </span>
                    )}
                  </button>
                    ...
                    <ul className="pl-8 mt-2 space-y-1">
                      {item.submenus.map((submenu, idx) => (
                        <li key={idx}>
                          <a
                            href="#"
                            className="block p-2 text-sm rounded-md hover:bg-purple-700/50"
                          >
                            {submenu}
                          </a>
                        </li>
      ...
      <div className={`flex-1 ${isSidebarOpen ? "ml-64" : "ml-20"} transition-all duration-300`}>
        ...
        <main className="p-6">
          <div className="mb-6">
            ...
            <p className="text-gray-600">Welcome back, Jane! Here's what's happening today.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-lg shadow">
              ...
              <div className="p-4 border-t border-gray-200 text-center">
                <button className="text-sm text-purple-600 hover:text-purple-800 font-medium">View All Appointments</button>
              </div>
            </div>
                  ...
                  <ul className="divide-y divide-gray-200">
                    {recentSales.map((sale) => (
                      <li key={sale.id} className="py-3 flex justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{sale.service}</p>
                          <p className="text-xs text-gray-500">{sale.date}</p>
                        </div>
                        <p className="text-sm font-semibold text-green-600">${sale.amount}</p>
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="p-4 border-t border-gray-200 text-center">
                  <button className="text-sm text-purple-600 hover:text-purple-800 font-medium">View All Sales</button>
                </div>
              </div>
            </div>
          </div>
